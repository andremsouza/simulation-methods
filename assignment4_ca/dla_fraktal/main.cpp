...
void does_it_stick() {
  ...
  if (r < p)
    ...
    if ((grid[randomwalker_x + 1][randomwalker_y] == 1) ||
      ...
      if (R_max_candidate2 > R_max * R_max) R_max = sqrt(R_max_candidate2);

      // update r_gyration_n
      r_gyration_n += sqrt(R_max_candidate2);
...
int main(int argc, const char *argv[]) {
  if (argc != 2) {
    std::cout << "usage: ./main p" << std::endl;
    exit(1);
  }
  p = atof(argv[1]);
