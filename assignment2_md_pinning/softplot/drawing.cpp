    ...
    for(i=0;i<global.N_objects;i++)
        ...
        if (inside_zoomed_box(x,y))
            ...
            if (color==2) glColor3f(1.0, 0.0, 0.0);
                ...
                //color 5 reserved for pinning sites
                else if (color==5) glColor3f(0.0, 0.0, 0.0);
            ...
            //transform particle coordinates into drawing coordinates
            transform_to_opengl(&x,&y);
        
            if (color<5) DrawCircle(x, y, global.radius_particle, 36,1);
            else DrawCircle(x, y, 0.04 , 36,0);//no fill (pinning site)
            
            //draw the trace of the particle
            //draw_trace_of_particle(frame,i);
...
/*
void draw_trace_of_particle(int frame, int i)
{
double x,y;
int j;
...
for(j=0;j<10;j++)
    {
    x = global.objects[frame-j][i].x;
    y = global.objects[frame-j][i].y;
    transform_to_opengl(&x,&y);
    glVertex3f(x,y,-10);
    }
glEnd();
