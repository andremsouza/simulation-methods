...
MD::Simulation::Simulation(bool t_verbose,
    ...
    if(m_verbose) {
        ...
        std::cout << "Screening length = " << 
            m_particle_particle_screening_length << std::endl;
        std::cout << "Driving force on particles = "
            << m_particle_driving_force << std::endl;
    }
    ...
    // Initializing files
    std::cout << "Opening file \"particles.mvi\"" << std::endl;
    m_moviefile.open("./particles.mvi",
...
void MD::Simulation::runSimulation(int t_total_time, int t_echo_time,
    ...
    for(m_time = 0; m_time < t_total_time; m_time++) {
        ...
        if(m_time % t_echo_time == 0) {
            std::cout << "Timestep: " << m_time << " / " << t_total_time
                << std::endl;
        }
    ...
    std::cout << "Run finished on: " << m_time_end.time_since_epoch().count()
        << std::endl;
    std::cout << "Program running time was: "
