...
void initialize_global_data(char *filename) {
  // open file and read first frame, for parameter initialization
  strcpy(global.moviefilename, filename);
  open_movie_file();
  dummy_read_cmovie_frame();
  fclose(global.moviefile);
  global.Windowsize_x = 800;
  global.Windowsize_y = 800;

  global.SX = global.SY = round(sqrt(global.N_objects / PARTICLE_DENSITY));
